...
print("Generating dynamic video clips...")
...
for idx, chapter in enumerate(data):
    ...
    # Re-combine punctuation with the sentence
    sentences_paired = []
    ...
    for s_idx, sentence in enumerate(sentences_paired):
        ...
        # Load background and darken it
        bg_img = Image.open(bg_path).resize((1920, 1080))
        dark_layer = Image.new('RGBA', bg_img.size, (0, 0, 0, 150))
        bg_img = bg_img.convert('RGBA')
        bg_dark = Image.alpha_composite(bg_img, dark_layer)
        bg_dark = bg_dark.convert('RGB')
        ...
        # Draw Sentence (Wrap text if too long)
        def wrap_text(text, font, max_width):
            lines = []
            words = list(text) # For Chinese, word is character
            current_line = ""
            for char in words:
        ...
        # Create ImageClip for this sentence
...
output_file = "C:\\Users\\manpo\\OneDrive\\桌面\\114下學期期末考動態精華影片.mp4"
...
print("Done! Dynamic Video Generated.")
